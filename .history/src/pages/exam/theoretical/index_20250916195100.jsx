...
export default function TheoreticalExamPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      try {
        const response = await fetch("https://mot-palestine-api.onrender.com/api/mot/theoretical", {
          method: "POST",
          headers: {
        ...
        console.log("Public API failed, trying alternative method...");
      }

      // Fallback: Use CORS proxy to access MOT directly
      const corsProxy = "https://api.allorigins.win/raw?url=";
      const motUrl = encodeURIComponent("https://www.mot.gov.ps/mot_Ser/Exam.aspx");
      
      // First, get the initial page
      const initialResponse = await fetch(corsProxy + motUrl);
      if (!initialResponse.ok) {
        throw new Error("فشل في الاتصال بخدمة النتائج");
      }
      ...
      // Submit the form
      const submitResponse = await fetch(corsProxy + motUrl, {
        method: "POST",
        headers: {
      ...
      console.error(err);
      setError(err.message || "حدث خطأ أثناء جلب نتيجة الامتحان. يرجى المحاولة مرة أخرى.");
    }
    setLoading(false);
  };
