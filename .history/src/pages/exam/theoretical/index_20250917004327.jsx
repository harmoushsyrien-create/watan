...
export default function TheoreticalExamPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!result.found || !result.data || !result.data.rows) {
        setExamResult({});
      } else {
      ...
      let errorMessage = err.message || "حدث خطأ أثناء جلب نتيجة الامتحان.";
      
      // Provide more specific error messages
      if (errorMessage.includes('Failed to fetch') || errorMessage.includes('fetch')) {
        errorMessage = "لا يمكن الاتصال بخوادم وزارة المواصلات. يرجى المحاولة لاحقاً أو زيارة الموقع مباشرة.";
        setShowCorsHelp(true);
      } else if (errorMessage.includes('timeout') || errorMessage.includes('Timeout')) {
        errorMessage = "انتهت مهلة الاتصال. يرجى المحاولة مرة أخرى.";
      } else if (errorMessage.includes('404') || errorMessage.includes('Not Found')) {
        errorMessage = "لا يمكن العثور على صفحة الامتحان. يرجى المحاولة لاحقاً.";
      } else if (errorMessage.includes('500') || errorMessage.includes('Internal Server Error')) {
        errorMessage = "خطأ في خوادم وزارة المواصلات. يرجى المحاولة لاحقاً.";
      }
