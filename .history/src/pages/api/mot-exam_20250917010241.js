...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      // Try proxy service for Vercel to bypass network restrictions
      if (process.env.NODE_ENV === 'production') {
        console.log('Trying proxy service for Vercel...');
        
        try {
          // Use a public proxy service to bypass network restrictions
          ...
          console.error('Proxy service failed:', proxyError.message);
          
          // Try alternative proxy
          try {
            console.log('Trying alternative proxy...');
            const altProxyUrl = 'https://cors-anywhere.herokuapp.com/https://www.mot.gov.ps/mot_Ser/Exam.aspx';
            
            const altProxyResponse = await axios.get(altProxyUrl, {
              headers: {
            ...
            initialResponse = altProxyResponse;
            initialHtml = altProxyResponse.data;
